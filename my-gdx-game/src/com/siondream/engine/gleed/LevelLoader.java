...
package com.siondream.engine.gleed;
...
public class LevelLoader extends AsynchronousAssetLoader<Level, LevelLoader.GLEEDParameter > implements Disposable {
		...
		try {
			...
			if (m_level == null) {
				...
				m_level.properties.load(root);
			      ...
			      m_level.setPhysicsSystem(system);
			}
	...
	public Array<AssetDescriptor> getDependencies(String fileName, GLEEDParameter parameter) {
		...
		Array<String> texture_names = new Array<String>();
			...
			if (!m_atlasFile.isEmpty()) {
				...
				for (int i = 0; i < elements.size; ++i) {
					...
					if (element.getAttribute("xsi:type", "").equals("TextureItem")) {
						...
						AssetDescriptor desc = new AssetDescriptor(builder.toString(), Texture.class);
						if ( !texture_names.contains(builder.toString(),false))
						{
							s_logger.info("texture dependency " +  builder.toString());
							dependencies.add(desc);
							texture_names.add(builder.toString());
	...
	private void loadLayer(Element element) {
		...
		if (layer.name.contains("Dynamic_Collision"))
		{
			dynamic = true;
		}
		else if (layer.name.contains(("Static")))
				{
		...
		for (int i = 0; i < items.size; ++i) {
			...
			if (type.equals("TextureItem")) {
				...
				layer.objects.add(rectangle);// todo, prob should make this shit cleaner too
				
				// TODO REDO THIS
				// todo need to pair graphics with body
				if ( dynamic)
				{
					m_level.getPhysicsSystem().createDynamicRectangle(rectangle);
				}
				
				if ( static_collision )
				{
					m_level.getPhysicsSystem().createStaticRectangle(rectangle);
				}
				...
				if ( dynamic)
				{
					m_level.getPhysicsSystem().createDynamicCircle(circle);
				}
				
				if ( static_collision )
				{
					m_level.getPhysicsSystem().createStaticCircle(circle);
				}
			}
		}
